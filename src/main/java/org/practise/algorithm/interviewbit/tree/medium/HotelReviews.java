...
public class HotelReviews {
    public List<Integer> solve(String goodwords, List<String> words) {
        TrieNode trienode = new TrieNode();
        for (String goodword : goodwords.split("_")) {
            trienode.insert(goodword);
        }
        ...
        for (int index = 0; index < words.size(); index++) {
            ...
            for (String w : word.split("_")) {
                if (trienode.search(w)) {
                    goodWordCount++;
                }
            }
            if (! sortedMap.containsKey(goodWordCount)) {
                sortedMap.put(goodWordCount, new ArrayList<>());
            }
            sortedMap.get(goodWordCount).add(index);
        }

        List<Integer> resultList = new ArrayList<>();
        for (int i = sortedMap.lastKey(); i > sortedMap.firstKey();) {
            resultList.addAll(sortedMap.get(i));
            i = sortedMap.lowerKey(i);
        }
        resultList.addAll(sortedMap.get(sortedMap.firstKey())); // add lowest key at last
        return resultList;
    }
    ...
    class TrieNode {
        ...
        public boolean search(String word) {
            final TrieNode node = prefixSearch(word);
