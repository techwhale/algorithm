...
public class ExclusiveTimeOfFunctions {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        stack.push(log.id);
        int previousTime = log.time, i = 1;

        while (i < logs.size()) {
            log = parseLog(logs.get(i));
            if (log.start) {
                if (! stack.isEmpty()) {
                    result[stack.peek()] += log.time - previousTime;
                }
                stack.push(log.id);
                previousTime = log.time;
            } else {
                result[stack.peek()] += log.time - previousTime + 1;
                stack.pop();
                previousTime = log.time + 1;
            }
            i++;
        }
        return result;
    ...
    private Log parseLog(String log) {
        String[] l = log.split(":");
        return new Log(Integer.parseInt(l[0]), l[1].equals("start"), Integer.parseInt(l[2]));
    }
