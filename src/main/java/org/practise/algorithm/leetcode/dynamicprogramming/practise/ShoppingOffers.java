...
public class ShoppingOffers {
    ...
    private int shopping(List<Integer> price, List<List<Integer>> special, List<Integer> needs, Map<List<Integer>, Integer> memo) {
        ...
        int totalCost = computeCost(price, needs);
        for (int i = 0; i < special.size(); i++) {
            final List<Integer> offer = special.get(i);
            ...
            if (j == needs.size()) {
                totalCost = Math.min(totalCost, offer.get(j) + shopping(price, special, clone, memo));
            }
        }
    ...
    private int computeCost(List<Integer> price, List<Integer> needs) {
        int totalCost = 0;
